...
pub enum PathAttribute {
    ...
    AsPath(AsPath),
    NextHop(Ipv4Addr),
    DontKnow(Vec<u8>), // 対応していないPathAttribute用
}
    ...
    pub fn from_u8_slice(
        ...
        while bytes.len() > i {
            ...
            let attribute_length = if attribute_length_octets == 1 {
                ...
                u16::from_be_bytes(
                    bytes[i + 2..i + 4].try_into()
                    .context("aaa")?) as usize
            };
            ...
            let path_attribute = match attribute_type_code {
                1 => PathAttribute::Origin(
                    Origin::try_from(bytes[attribute_start_index]
                )?),
...
impl From<&PathAttribute> for BytesMut {
    fn from(p: &PathAttribute) -> BytesMut {
        ...
        // PathAttrubuteのBytes表現は以下の通り
        // [Attribute Flag (1 octet)]
        // [Attribute Type Code (1 octet)]
        // [Attribute Length(1 or 2 octet)]
        ...
        // - 4bit目: Attribute Lengthがone octetなら0, two octetなら1
        // - 5-8bit目:使用しない。ゼロ
        match p {
...
impl AsPath {
    ...
    fn bytes_len(&self) -> usize {
        ...
        // AsSetかAsSequenceかを表すoctet + as の数を表すoctet + asのbytesの値
        1 + 1 + as_bytes_length
    }
}
