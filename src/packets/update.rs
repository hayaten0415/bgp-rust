...
use crate::routing::{AdjRibOut,RibEntry,Ipv4Network};
use crate::path_attribute::{PathAttribute, AsPath, Origin};
...
pub struct  UpdateMessage {
    ...
    withdrawn_routes_length: u16, // ルート数ではなく、byteにしたときのオクテット数
    pub path_attributes: Arc<Vec<PathAttribute>>,
    pub path_attributes_length: u16, // bytesにした時のオクテット数。
...
impl UpdateMessage {
    pub fn new(
        ...
        let path_attributes_length = path_attributes.iter().map(|p| p.bytes_len()).sum::<usize>() as u16;
...
impl TryFrom<BytesMut> for UpdateMessage {
    ...
    fn try_from(bytes: BytesMut) -> Result<Self, Self::Error> {
        ...
        let withdrawn_routes_length: u16 = u16::from_be_bytes(bytes[19..21].try_into().context(format!(
        ...
        let path_attribute_start_index = withdrawn_routes_end_index + 2;
        let total_path_attribute_length = u16::from_be_bytes(
            bytes[withdrawn_routes_end_index..path_attribute_start_index]
            .try_into()
            .context(format!(
        ...
        let path_attribute_bytes = &bytes[path_attribute_start_index..
            path_attribute_start_index + total_path_attribute_length as usize];
        let path_attributes = Arc::new(PathAttribute::from_u8_slice(path_attribute_bytes)?);
        let nlri_start_index = path_attribute_start_index + total_path_attribute_length as usize;
        let network_layer_reachability_information = Ipv4Network::from_u8_slice(&bytes[nlri_start_index..])?;
