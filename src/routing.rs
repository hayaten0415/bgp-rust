...
use crate::path_attribute::{PathAttribute, Origin, AsPath};
...
/// AdjRibIn / LocRib / AdjRibOutで同じようなデータ構造・処理を持つため、
/// 共通の処理はRib構造体に実装し、これら3つの構造体のメンバにはRib構造体を持たせる。
/// 
/// Rib Entryは、3つのRibを渡りながら処理される。
/// Rib間で受け渡す時にCloneを避けたいため、HashMapのKeyは
/// Arc<RibEntry>にしている。
#[derive(Debug, PartialEq, Eq, Clone)]
...
impl AdjRibOut {
    ...
    /// Vec<UpdateMessage>の戻り値にしている。
    pub fn create_update_messages(
        &self,
        local_ip: Ipv4Addr,
        local_as: AutonomousSystemNumber,
    ) -> Vec<UpdateMessage> {
        // PathArributeをKeyに、Vec<IPv4Network>をValueのHashMapを使って、
        // 同じPathAttributeのNLRIは同じVec<IPv4Network>に纏めている。
        ...
        for(path_attributes, routes) in hash_map.into_iter() {
            ...
            // PathAttributeを２つ変更する。local ip, as_path add;
...
impl Ipv4Network {
    pub fn new(
        ...
        let net = ipnetwork::Ipv4Network::new(addr, prefix)
            .context(format!(
                "Ipv4NetworkをConstruct出来ませんでした。
                addr: {}, prefix: {}",
                addr, prefix
            ))?;
        Ok(Self(net))
        ...
        while bytes.len() > i {
            ...
            if prefix == 0 {
                ...
                return Err(ConvertBytesToBgpMessageError::from(anyhow::anyhow!(
                    "bytes -> Ipv4に変換できませんでした。Prefixが0-32の間ではありません。"
                )));
            };
        }
        Ok(networks)
...
impl LocRib {
    pub async fn new(config: &Config) -> Result<Self> {
        let path_attributes = Arc::new(vec![
            ...
            // AsPathは、他のピアから受信したルートと統一的に扱うために、
